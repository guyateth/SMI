/**
	Treecast test. A sequeuence of number is broadcasted.
...
{
    char check=1;
    SMI_TreecastChannel  __attribute__((register)) chan= SMI_Open_treecast_channel(N, SMI_INT,0, root,comm);
    for(int i=0;i<N;i++)
    {

        int to_comm=i;
        SMI_Treecast(&chan,&to_comm);
        check &= (to_comm==i);
    }
    *mem=check;
}
...
__kernel void test_float(__global char* mem, const int N, char root,SMI_Comm comm)
{
    ...
    SMI_TreecastChannel  __attribute__((register)) chan= SMI_Open_treecast_channel(N, SMI_FLOAT,1, root,comm);
    for(int i=0;i<N;i++)
    {
...
__kernel void test_double(__global char* mem, const int N, char root,SMI_Comm comm)
{
    ...
    SMI_TreecastChannel  __attribute__((register)) chan= SMI_Open_treecast_channel(N, SMI_DOUBLE,2, root,comm);
    for(int i=0;i<N;i++)
    {
...
__kernel void test_char(__global char* mem, const int N, char root,SMI_Comm comm)
{
    char check=1;
    char expected=root;
    SMI_TreecastChannel  __attribute__((register)) chan= SMI_Open_treecast_channel(N, SMI_CHAR,3, root,comm);
    for(int i=0;i<N;i++)
    {
...
__kernel void test_short(__global char* mem, const int N, char root,SMI_Comm comm)
{
    char check=1;
    short expected=root;
    SMI_TreecastChannel  __attribute__((register)) chan= SMI_Open_treecast_channel(N, SMI_SHORT,4, root,comm);
    for(int i=0;i<N;i++)
    {
