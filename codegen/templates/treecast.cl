...
__kernel void smi_kernel_bcast_{{ op.logical_port }}(char num_rank)
{
    ...
    while (true)
    {
        if (stage == 0) // read from the application
        {
            ...
            {
                stage = 1;
                printf("END OF STAGE 0 ROOT; %d %d %d %d %d", my_rank, my_parent, child_one, child_two, num_requests);
            }
            else // i am not the root
            {
                stage = 2;
                printf("END OF STAGE 0 NONR; %d %d %d %d %d", my_rank, my_parent, child_one, child_two, num_requests);
            }
        ...
        else if (stage == 1) // handle the request for root (Initial data)
        {
            ...
            printf("GOT TREECST DATA; %d %d %d %d", my_rank, my_parent, child_one, child_two);
        }
        ...
        {   
            ...
            printf("SENT RR; %d %d %d %d", my_rank, my_parent, child_one, child_two);
            stage = 3;
        }
        else if (stage == 3) // wait for readies from all children
        {
            ...
            {
                printf("WAITING RR; %d %d %d %d %d", my_rank, my_parent, child_one, child_two, received_request);
                SMI_Network_message req = read_channel_intel({{ op.get_channel("ckr_control") }});
                printf("GOT RR; %d %d %d %d %d", my_rank, my_parent, child_one, child_two, received_request);
                received_request--;
            }
            else 
            {
        ...
        else if (stage == 4) // send data to children
        {
            ...
            if (!sent_one && child_one != -1)
            {
                ...
                printf("ROOT SENT CH1; %d %d %d %d", my_rank, my_parent, child_one, child_two);
                sent_one = true;
            }
            else if (!sent_two && child_two != -1) // this elseif makes sure only one packet is sent per loop iteration
            {
                ...
                SET_HEADER_PORT(mess_data.header, {{ op.logical_port }});
                write_channel_intel({{ op.get_channel("cks_data") }}, mess_data);
                printf("ROOT SENT CH2; %d %d %d %d", my_rank, my_parent, child_one, child_two);
                sent_two = true;
            }
            else
            {
        ...
        {
            mess_data = read_channel_intel({{ op.get_channel("treecast_data") }});
            printf("GOT FROM APP; %d %d %d %d", my_rank, my_parent, child_one, child_two);
            SET_HEADER_OP(mess_data.header, SMI_BROADCAST);
            stage = 4;
        }
        ...
        else if (stage == 6) // wait for new data
        {
            ...
            printf("GOT FROM PARENT; %d %d %d %d", my_rank, my_parent, child_one, child_two);
            SET_HEADER_OP(mess_data.header, SMI_BROADCAST);
            stage = 7;
        }
